...
# =====================
# Try to use Render disk if available
DATA_DIR = "/data"
LOCAL_STATE_FILE = "vb_state.json"
if os.path.isdir(DATA_DIR) and os.access(DATA_DIR, os.W_OK):
    STATE_FILE_PATH = os.path.join(DATA_DIR, "vb_state.json")
else:
    # fallback to local folder if /data not available or writable
...
def load_state():
    ...
    try:
        ...
        virtual_balance = vb
        log(f"✅ Loaded state from {STATE_FILE_PATH} (Last saved: {st.get('last_saved', 'unknown')})")
